...
namespace Server.CoreServerFunctionality
{
    ...
    {
        public static ObservableCollection<ServerSession> _sessions = new();
        private static SemaphoreSlim semaphoreSlim = new SemaphoreSlim(1,1);
        public static async Task HandleClient(TcpClient client)
        {
            var clietnAuthenticator = new AuthenticatedClient(client);
            var session = new ServerSession(clietnAuthenticator);
        }

        public static async Task UpdateClients(ServerSession session)
        {
            ...
            //ApplicationHelperMethods.FireEvent(_sessions, nameof(_sessions.CollectionChanged), session, new NotifyCollectionChangedEventArgs(NotifyCollectionChangedAction.Replace, new List<ServerSession> { session, }, index, index));
            semaphoreSlim.Release();
        }
    }

    public class AuthenticatedClient : IConnectionProperties
    {
        public TcpClient Client { get; set; }
        public SslStream SslStream { get; set; }
        public AuthenticatedClient(TcpClient client)
        {
            Client = client;
            AuthenticateClient();
        }
